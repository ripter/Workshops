...
var _toggleById = __webpack_require__(6);

var _toggleById2 = _interopRequireDefault(_toggleById);
 ...
 * It can only be changed by calling the `action` method.
 * @type {Store}
 */
var Store = function () {
  function Store(initalState, onChange) {
    ...
    this.reducers = [_toggleById2.default];
  }
  ...
  _createClass(Store, [{
    ...
    value: function action(actionObj) {
      ...
      state = this.perform(_state, actionObj);
      // perform all the pending actions.
      while (_pendingActions.length > 0) {
        var nextActionObj = _pendingActions.pop();
        state = this.perform(state, nextActionObj);
      }
    ...
    /**
     * Runs the reducers returning the new state.
     * Reducers can add items to _pendingActions array
     * @param  {Object} state
    ...
    key: 'perform',
    value: function perform(state, actionObj) {
      ...
      return reducers.reduce(function (acc, reducer) {
        return reducer(acc, actionObj, pushNextAction);
      }, state);
    }
    ...
    key: 'getState',
    value: function getState(path, defaultValue) {
...
exports.default = toggleById;

var _consts = __webpack_require__(0);

/**
 * Toggles a toggle by id.
 * @param  {Object} state - {toggles: {checked, toggleList}}
 * @param  {Object} action
 * @return {Object}
 */
function toggleById(state, action) {
  if (action.type !== _consts.ACTION.TOGGLE) {
    return state;
  }
  var id = action.id;

  var toggle = state.toggles[id];

  // toggle by id
  state.toggles[id].checked = !toggle.checked;

  if (!toggle.toggleList) {
    return state;
  }

  // toggle each item in the toggleList
  toggle.toggleList.forEach(function (toggleId) {
    state.toggles[toggleId].checked = !state.toggles[toggleId].checked;
  });
...
var _domlens = __webpack_require__(1);

var _domlens2 = _interopRequireDefault(_domlens);
...
// import domLens from 'domlens/src/domLens.js';
...
// Creates lens that can update the dom.
// It provides a way to map elements to state.
var lens = {
  '.toggle input': {
    checked: function checked(elm) {
      var id = elm.id;
      var isChecked = store.getState('toggles.' + id + '.checked');
      return isChecked;
    },
...
function updateDom() {
  // custom forEach for the lens.
  // const forEach = (element) => {
  //   debugger;
  // };
  // domLens(forEach, lens);

  (0, _domlens2.default)(lens, store.getState());
}
// update the dom on store change
store.onChange = updateDom;
// render the inital store
updateDom();
